...
- **预览中调整图片与图表**：点击正文图片或 Mermaid 图表，通过悬浮手柄调整宽高、切换对齐，还可为 Mermaid 选择单图主题；操作会写回 Markdown 并重新分页。
- **自动分页排版**：根据页面尺寸处理段落、列表、表格、代码块和图片，支持手动分页符。
- **完整文档结构**：可配置封面、目录、页眉、页脚、页码和标题层级样式；封面条目及页眉页脚可引用文档信息，页眉页脚还可显示当前章节标题。
...
4. 选择双栏、专注编辑或 A4 全屏视图检查最终分页效果；在双栏模式下双击编辑内容可定位预览，也可使用文档大纲和页码导航（可直接输入目标页码，或跳转首页 / 尾页）。点击预览中的正文图片或 Mermaid 图表，还可使用右侧 / 底部手柄调整宽高，在悬浮工具条输入尺寸、切换对齐，或为 Mermaid 选择单图主题；确认后会修改对应 Markdown 并重新分页。
5. 从右侧面板配置封面、页眉页脚、目录、配色和正文样式；封面条目值及页眉页脚文字可通过输入框内右侧的变量选择器引用文档信息，页眉页脚还可引用章节标题。在「样式」的「其他」页签可启用文本或图片水印，并设置居中或平铺、透明度、旋转及封面显示。
...
在封面条目的值、页眉和页脚的左 / 中 / 右文字中，可通过输入框内右侧的「＋」填入 `@title`、`@subtitle`、`@author`、`@department`、`@organization`、`@date`、`@number` 或 `@version`，引用对应的文档信息。页眉页脚还支持 `@h1`（一级标题）和 `@h2`（二级标题）。预览及 HTML / PDF 中，当前章节尚无二级标题时 `@h2` 使用一级标题；封面没有正文章节，目录页使用目录标题。

字段必须单独占满整个输入框，例如 `@title` 会解析，`报告：@title` 不会解析；选择器会替换原有文字，不会在光标处插入。A4 预览与 HTML / PDF 按页面显示章节标题；Word 导出的章节字段使用 Word 标题引用域，仅整篇文档没有二级标题时 `@h2` 才回退到一级标题，章节交界处的结果可能与预览不同。
...
在 A4 预览中选中图片也能调整尺寸与对齐；只指定宽度时保持图片比例，宽高同时指定时按设定尺寸拉伸。滚动预览或开合设置面板会关闭悬浮控制层。仅支持 Markdown 正文图片，不处理封面、页眉或原生 HTML 图片。
...
也可点击 A4 预览中的图表，用悬浮工具条选择单图主题和对齐，或拖动右侧 / 底部手柄调整尺寸；操作完成后会改写该图表的围栏首行。图内已有 `init` 主题指令时，工具条不会覆盖它。
