...
import { DEFAULT_TEMPLATE_THEME_ID, DOCUMENT_TEMPLATE_CATEGORIES, DOCUMENT_TEMPLATES, resolveDocumentTemplate } from './documentTemplates';
...
describe('DOCUMENT_TEMPLATES', () => {
  ...
  it('业务分类 id 唯一且每个分类至少包含一个模板', () => {
    const categoryIds = DOCUMENT_TEMPLATE_CATEGORIES.map((category) => category.id);
    expect(new Set(categoryIds).size).toBe(categoryIds.length);
    for (const categoryId of categoryIds) {
      expect(DOCUMENT_TEMPLATES.some((template) => template.category === categoryId)).toBe(true);
    }
  });
  ...
  it('每个内置主题至少有一个演示模板', () => {
    const standaloneTemplateIds = new Set(['blank', 'system-guide']);
    const themeIds = DOCUMENT_TEMPLATES
      .filter((template) => !standaloneTemplateIds.has(template.id))
      .map((template) => template.recommendedThemeId);
    expect(new Set(themeIds)).toEqual(presetThemeIds);
  });
...
describe('resolveDocumentTemplate', () => {
  ...
  it('应用模板差异项：关闭封面与目录、一级标题居中', () => {
    const resolved = resolveDocumentTemplate(meetingMinutes, baseTheme);
    expect(resolved.theme.cover.showCover).toBe(false);
    expect(resolved.theme.toc.show).toBe(false);
    expect(resolved.theme.style.h1Center).toBe(true);
  });

  it('模板未声明的字段完全沿用推荐主题', () => {
    const resolved = resolveDocumentTemplate(meetingMinutes, baseTheme);
    expect(resolved.theme.style).toEqual({ ...baseTheme.style, h1Center: true });
    expect(resolved.theme.toc).toEqual({ ...baseTheme.toc, show: false });
    expect(resolved.theme.header).toEqual(baseTheme.header);
    expect(resolved.theme.footer).toEqual(baseTheme.footer);
  ...
  it('未声明差异项的模板保持推荐主题的封面与目录配置', () => {
    ...
    const resolved = resolveDocumentTemplate(blank, theme);
